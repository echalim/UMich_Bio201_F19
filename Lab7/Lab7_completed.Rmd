...
# save richness data 
write_delim(samp_data, path = "~/Documents/UMich_Bio201_F19/Lab7/curated_data/samp_data.txt",
...
wk1_3_df <- inner_join(x = wk1, y = wk3,
                    by = c("participant_id", 
...
t.test(x = wk1_3_df$avg_richness_wk1, y = wk1_3_df$avg_richness_wk3, 
...
Since p-value is 1 (>0.05), we would fail to reject the null hypothesis that the richness changed from week 1 to week 3 in indiviuals who consumed BRMPS. 
...
save_plot(filename = "Lab7/figures/rich2.pdf",
          plot = rich2,
          nrow = 1, ncol = 2, 
...
# write code for paired t-test here 
wk1_3_simpson_df <- inner_join(x = wk1, y = wk3,
                    by = c("participant_id", 
                           "semester", "supplement_consumed")) %>%
  rename(avg_rich_wk1 = avg_richness.x,
         avg_rich_wk3 = avg_richness.y) %>%
  select(-starts_with("study_week"))

t.test(x = wk1_3_simpson_df$avg_rich_wk1, y = wk1_3_simpson_df$avg_rich_wk3, 
       alternative = "less", paired = TRUE) 

# p-value = ?
#p-value is 9.946e-12. Since p-value is <0.05, we reject the null hypothesis that the Shannon Diversity differed significantly before vs. during supplement consumption. 
```
...
save_plot(filename = "Lab7/figures/simp2.pdf",
          plot = simp2,
          nrow = 1, ncol = 2, 
