...
knitr::opts_knit$set(root.dir = "~/Documents/UMich_Bio201_F19/echalim")
```
...
```{r}
scfa_data <- read_delim(file = "Lab3/raw_data/SCFA_data.txt",
...
propionate_wide <- scfa_wkly %>%
  select(participant_id, semester, supplement_consumed, study_week, mean_proprionate) %>% #drop extra columns
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
...
ggplot(but_avgdiff, aes(x = supplement_consumed,
                     y = delta_mean_but)) +
  geom_jitter()
```
...
```{r}
# write code here
total_scfas <- inner_join(acetate_wide, butyrate_wide, by = c("participant_id", "semester", "supplement_consumed")) %>%
  full_join(proprionate_wide, by = c("participant_id", "semester", "supplement_consumed")) %>% 
  mutate(total_wk1 = ace_wk1 + but_wk1 + pro_wk1, total_wk3 = ace_wk3 + but_wk3 + pro_wk3, delta_total = total_wk3 - total_wk1)
```
...
```{r}
# write code here
ggplot(data = total_scfas, aes(x = supplement_consumed, y = delta_total, color = semester)) + geom_point() + facet_grid(~semester, scales = "free")
...
but_long <- total_scfas %>%
  ...
  rename(week1 = but_wk1, week3 = but_wk3) %>%
  gather(key = study_week, vavlue = butyrate, week1, week3)
...
pro_long <- total_scfas %>%
  ...
  rename(week1 = pro_wk1, week3 = pro_wk3) %>% 
  gather(key = study_week, value = proprionate, week1, week3)
...
```{r}
scfa_long <- full_join(ace_long, but_long, by = c("participant_id", "semester", "supplement_consumed", "study_week")) %>%
...
```{r}
# for data frames:
write_delim(total_scfas, path = "Lab4/curated_data/total_scfas_wide.txt", delim = "\t", na = "NA", col_names = TRUE)
