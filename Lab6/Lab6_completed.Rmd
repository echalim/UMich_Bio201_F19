...
#inner_join because we want to include only categories that are common
```
...
write_delim(tuk_wk1, path = "~/Documents/UMich_Bio201_F19-master/Lab6/curated_data/tuk_wk1.txt",
...
```{r}
# import methane data 
breath_wkly <- read_delim("~/Documents/UMich_Bio201_F19-master/Lab5/curated_data/breath_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
# format methane data
  rename_all(tolower) %>%
...
```{r}
# hypothesis: 
# Hydrogen breath levels will difer between participants consuming fiber supplements. 
# plot 
breath_wkly %>%
  ggplot(aes(x = supplement_consumed, 
             y = h2_mean, 
...
s1 <- breath_wkly %>%
  ...
  ggplot(s1, aes(x=h2_mean)) +
    geom_histogram() 
  labs(x = "h2_mean_Accessible" ,
       y = "count")
#p-value 0.0937 so normal distribution because >0.05 
...
s2 <- breath_wkly %>%
  ...
  ggplot(s2, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 0.1695 so normal distribution because >0.05
...
s3 <- breath_wkly %>%
  ...
  ggplot(s3, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 0.04114 so non-normal distribution because <0.05
...
s4 <- breath_wkly %>%
  ...
  ggplot(s4, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 2.823e-12 so non-normal distribution because <0.05
...
s5 <- breath_wkly %>%
  ...
  ggplot(s5, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 0.05314 so nomral distribution because >0.05
...
s6 <- breath_wkly %>%
  ...
  ggplot(s6, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 9.956e-06 so non-normal distribution because <0.05
...
s7 <- breath_wkly %>%
  ...
  ggplot(s7, aes(x=h2_mean)) +
    geom_histogram()
#p-value 0.001864 so non-normal distribution because <0.05
...
s8 <- breath_wkly %>%
  ...
  ggplot(s6, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 0.0002929 so non-normal distribution because <0.05 
...
s9 <- breath_wkly %>%
  ...
  ggplot(s9, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 7.089e-05 so non-normal distribution because <0.05
  
s10 <- breath_wkly %>%
  filter(supplement_consumed == "Psyllium")
  shapiro.test(s10$h2_mean) #p-value 0.94209
  ggplot(s10, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 0.94209 so normal distribution because >0.05
...
s11 <- breath_wkly %>%
  ...
  ggplot(s11, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 0.472 so normal distribution because >0.05
...
s12 <- breath_wkly %>%
  ...
  ggplot(s12, aes(x=h2_mean)) +
    geom_histogram() 
#p-value 7.102e-07 so non-normal distribution because <0.05
  
bartlett.test(h2_mean ~ supplement_consumed, data = breath_wkly)
#p-value 6.83e-08

#This data set has unequal variances because the p-value of some of the supplements consumed are normal while some are non-normal. 
```
...
```{r}
# conclusion 
#There is no significant difference because it rejects the null hypothesis. Also, large p-value does not indicate that at least one group is different from the others. 
```
...
tax_ratio <- inner_join(Fr, Br) %>%
  ...
  tax_ratio_1 <- full_join(tax_ratio, scfa_ratio)  #join with scfa_ratio data frame  
```                 
...
Fr_abundance <- tax_ratio_1 %>%
  ...
  ylab("Relative abundance (firmicutes abundance)") 

save_plot(filename = "Lab6/figures/Fr_abundance.pdf",
          plot = Fr_abundance,
          nrow = 1, ncol = 2, 
...
#conclusion: This p-value is low (p-value < 2.2e-16), so we do have significance. However, when we look at the adjusted R-squared value which indicates the quality of linear fit (R-squared = 0.003863), it is not high since it is lower than 0.8. Hence, there might just be a small relationship between the butyrate concentration and Firmicutes abundance. 
...
```{r}
# plot
f_b_abundance <- tax_ratio_1 %>%
  ggplot(aes(x = butyrate_ratio,
             y = f_b_ratio)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + xlab("butyrate_ratio (mmol/kg)") + ylab("Relative abundance (f_b_ratio)")


save_plot(filename = "Lab6/figures/f_b_abundance.pdf",
          plot = f_b_abundance,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
...
#conclusion: The same conclusions can be drawn for the relationship between butyrate ration and Firmicutes to Bacteroidetes ratio (f_b_ratio). 
...
```{r}
# plot
propionate_conc_Br <- tax_ratio_1 %>%
  ggplot(aes(x = propionate_mean,
             y = bacteroidetes_abund)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + xlab("propionate_mean (mmol/kg)") + ylab("Relative abundance(Bacteroidetes)")


save_plot(filename = "Lab6/figures/propionate_conc_Br.pdf",
          plot = propionate_conc_Br,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
...
#conclusion: This p-value is low (p-value < 2.2e-16), so we do have significance. However, when we look at the adjusted R-squared value which indicates the quality of linear fit (R-squared = 0.04201), it is not high since it is lower than 0.8. Hence, there might just be a small relationship between the propionate concentration and Bacteroidetes abundance. 
...
```{r}
# plot
propionate_f_b_ratio <- tax_ratio_1 %>%
  ggplot(aes(x = propionate_ratio,
             y = f_b_ratio)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + xlab("propionate_ratio (mmol/kg)") + ylab("Relative abundance(f_b_ratio)")


save_plot(filename = "Lab6/figures/propionate_f_b_ratio.pdf",
          plot = propionate_f_b_ratio,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
...
#conclusion: The same conclusion can be drawn for the relationship between propionate ration and Firmiutes to Bacteroidetes ratio. 
```
...
pH_BSS_wkly <- full_join(pH_data, BSS_data) 

# join 
tax_scfa_pH <- full_join(tax_ratio_1, pH_BSS_wkly) %>%
  na.omit(semester)
```
...
```{r}
# plot
total_scfa_acetate <- tax_scfa_pH %>%
  ggplot(aes(x = acetate_mean,
             y = totalscfa_mean)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + xlab("acetate mean (mmol/kg)") + ylab("Total SCFAs (mmol/kg)")


save_plot(filename = "Lab6/figures/total_scfa_acetate.pdf",
          plot = total_scfa_acetate,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
...
#conclusion: This p-value is low (p-value < 2.2e-16), so we do have significance. However, when we look at the adjusted R-squared value which indicates the quality of linear fit (R-squared = 0.8716), it is  high since it is higher than 0.8 which means it is a high quality fit. Hence, there is a relationship between the total SCFAs and acetate. 
```

Do you draw the same conclusions if you use the total SCFAs and acetate ratio?
```{r}
# plot
total_scfa_acetate_ratio <- tax_scfa_pH %>%
  ggplot(aes(x = acetate_ratio,
             y = totalscfa_mean)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + xlab("acetate ratio (mmol/kg)") + ylab("Total SCFAs (mmol/kg)")


save_plot(filename = "Lab6/figures/total_scfa_acetate_ratio.pdf",
          plot = total_scfa_acetate_ratio,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
...
#conclusion: This p-value is low (p-value < 2.2e-16), so we do have significance. However, when we look at the adjusted R-squared value which indicates the quality of linear fit (R-squared = 0.01794), it is low since it is lower than 0.8 which means it is a low quality fit. Hence, there is might only be a small relationship between the total SCFAs and acetate. 
```
...
```{r}
# plot
total_scfa_pH <- tax_scfa_pH %>%
  ggplot(aes(x = ph,
             y = totalscfa_mean)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + xlab("ph") + ylab("Total SCFAs (mmol/kg)")


save_plot(filename = "Lab6/figures/total_scfa_pH.pdf",
          plot = total_scfa_pH,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
...
# correlation test 
m66a <- tax_scfa_pH %>%
  lm(ph ~ totalscfa_mean, data = .)

summary(m66a) #view results 

#conclusion: This p-value is low (p-value < 2.2e-16), so we do have significance. However, when we look at the adjusted R-squared value which indicates the quality of linear fit (R-squared = 0.07956), it is  low since it is lower than 0.8 which means it is a low quality fit. Hence, there might only be a small relationship between the total SCFAs and acetate. 
```
...
```{r}
# plot
butyrate_pH <- tax_scfa_pH %>%
  ggplot(aes(x = ph,
             y = butyrate_mean)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + xlab("ph") + ylab("butyrate mean (mmol/kg)")


save_plot(filename = "Lab6/figures/butyrate_pH.pdf",
          plot = butyrate_pH,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
...
# correlation test 
m67a <- tax_scfa_pH %>%
  lm(ph ~ butyrate_mean, data = .)

summary(m67a) #view results 

#conclusion: This p-value is low (p-value < 2.2e-16), so we do have significance. However, when we look at the adjusted R-squared value which indicates the quality of linear fit (R-squared = 0.0428), it is low since it is lower than 0.8 which means it is a low quality fit. Hence, there might only be a small relationship between the pH and butyrate. 
```
...
write_delim(scfa_ratio, 
            ...
            delim = "\t", quote = FALSE, col_names = TRUE)
write_delim(tax_ratio_1, 
            path="Lab6/curated_data/tax_ratio_1.txt",
            delim = "\t", quote = FALSE, col_names = TRUE)
