...
# Why do you think it happens? I think for it to be classified as numeric, all the things in the set must be numerical and even if there is only one character, it will classify it as character. 
```
...
# only 1 out of the 8 is "TRUE" as a character in combined_logical
```

You’ve probably noticed that objects of different types get converted into a single, shared type within a vector. In R, we call converting objects from one class into another class coercion. These conversions happen according to a hierarchy, whereby some types get preferentially coerced into other types. What is the order of the hierarchy? 
Since numerical class will be classified as a character class when both classes are combined, the character class has a higher hierarchy compared to the numerical class. 
...
supplement <- c("RPS", "RMS", "RPS", "Insulin", "Arabinoxylan", "RPS", "RPS", "RMS", "Insulin", "Accessible", "RPS", "HiMaize", "Banana", "HiMaize", "RPS", "RMS", "RPS", "RPS", "Oat", "Rice", "Potato")

# What type are each of these? 
# Both vectors are characters
```
