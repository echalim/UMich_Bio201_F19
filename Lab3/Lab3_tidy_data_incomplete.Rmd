...
nrow(scfa_indv) #4078 rows in the data sheet 
ncol(scfa_indv) #19 columns in the data sheet
dim(scfa_indv) #4078 and 19 are the diameter of the data
colnames(scfa_indv) #There are 19 different column names in the data 
rownames(scfa_indv) #There are 4078 different row names in the data
glimpse(scfa_indv) #Gives us a glimpse of what is under each column name in the data 
str(scfa_indv) #Gives us the numerical and character in the data sheet
```
...
filter(sample_wt_g > 0.10) %>% 
filter(notes != "frozen >24hrs") %>% 
mutate(acetate_mmol_kg = (acetate_mM*0.002)/sample_kg,  
       butyrate_mmol_kg = (butyrate_mM*0.002)/sample_kg, 
       propionate_mmol_kg = (propionate_mM*0.002)/sample_kg, 
       total_SCFA = butyrate_mmol_kg + propionate_mmol_kg + acetate_mmol_kg) %>%
round(acetate_mmol_kg, butyrate_mmol_kg, propionate_mmol_kg, total_SCFA digits = 2) %>%
